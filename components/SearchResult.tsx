    ...
    <div className="mb-8 max-w-[600px] group">
      <div className="flex items-center gap-2 mb-1 text-sm text-[#202124]">
        ...
        <div className="flex flex-col leading-tight">
          <span className="text-[#202124] text-sm">{breadcrumbs[0]}</span>
      ...
      <a href={url} target="_blank" rel="noopener noreferrer" className="block group/link">
        <h3 className="text-[#1a0dab] text-xl cursor-pointer group-hover/link:underline mb-1 visited:text-[#609]">
          {title}
        </h3>
      </a>

      <div className="text-[#4d5156] text-sm leading-relaxed">
        {date && <span className="text-[#5f6368] mr-2">{date} —</span>}
        {description}
      </div>
      ...
      {sitelinks && sitelinks.length > 0 && (
         <div className="mt-4 ml-4 flex gap-6">
           {sitelinks.map((link, idx) => (
             <a key={idx} href={link.url} className="text-[#1a0dab] hover:underline text-sm font-medium">
               {link.label}
             </a>
           ))}
         </div>
